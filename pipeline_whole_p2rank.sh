...
experiment_name="11_11_models_noFilter"
threads=24

dataset_train_name_short=chen11
dataset_train_name="${dataset_train_name_short}_filter_p2rank"
...
dataset_test_name_short=joined
dataset_test_name="${dataset_test_name_short}_filter_p2rank"
...
dataset_eval_name_short=coach420
dataset_eval_name="${dataset_eval_name_short}_filter_p2rank"
...
tasks="A"
features_list="unp_PTM,unp_glycosylation,unp_lipidation,unp_mod_res,unp_topology,unp_sec_str,unp_non_standard,unp_natural_variant,unp_compbias,aa,aa_pairs,hydropathy,polarity,polarity_binary,charged,aromaticity,mol_weight,H_bond_atoms,HSE_up,HSE_down,exposureCN,bfactor,bfactor_CA,pdbekb_sec_str,pdbekb_conservation,dynamine,efoldmine,depth,mobiDB,phi_angle,psi_angle,cis_peptide,unp_disulfid,lbs,aa_ratio,conservation,unp_variation"
#features_list="unp_variation

echo "Running experiment $experiment_name..." #todo more details

#copy conservation data
mkdir -p $data_train_dir/conservation
cp -r ${main_folder}/conservation_results/$dataset_train_name_short/*.json $data_train_dir/conservation
echo "Conservation data copied for $dataset_train_name"
mkdir -p $data_test_dir/conservation
cp -r ${main_folder}/conservation_results/$dataset_test_name_short/*.json $data_test_dir/conservation
echo "Conservation data copied for $dataset_test_name"
mkdir -p $data_eval_dir/conservation
cp -r ${main_folder}/conservation_results/$dataset_eval_name_short/*.json $data_eval_dir/conservation
